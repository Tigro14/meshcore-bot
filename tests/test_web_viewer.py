...
class TestWebViewerLoggingReadonlyFix:
    """Minimal fix: viewer file logging must not write into the install tree.

    Under the production service's `ProtectSystem=strict` sandbox, /opt is
    read-only. The viewer must route its file log to the configured
    [Logging] log_file directory (writable), and never create ./logs under
    the code root.
    """
    ...
    def _write_config(
        ...
        if log_file is not None:
            cfg["Logging"] = {"log_file": log_file, "log_level": "INFO"}
        config_path = str(config_dir / "config.ini")
        with open(config_path, "w") as fh:
            cfg.write(fh)
        return config_path
    ...
    def test_empty_log_file_is_console_only_no_logs_dir(self, tmp_path: Path) -> None:
        ...
        assert not any(
            isinstance(h, RotatingFileHandler) for h in v.logger.handlers
        ), "Expected no file handler when log_file is empty"
        assert not (config_dir / "logs").exists(), "Must not create ./logs under config dir"
    ...
    def test_log_file_writes_beside_bot_log(self, tmp_path: Path) -> None:
        ...
        assert any(
            isinstance(h, RotatingFileHandler) for h in v.logger.handlers
        ), "Expected a file handler when log_file is set"
        assert (log_dir / "web_viewer.log").exists(), (
            f"Expected viewer log beside bot log at {log_dir / 'web_viewer.log'}"
        )
        assert not (config_dir / "logs").exists(), "Must not create ./logs under config dir"
